...
public Action TimeKill(Handle timer)
{
	...
	PrintToChatAll("{green}[DEATHRUN] {default}%t", "TimeOver");
	return Plugin_Continue;
}
...
public void NewRandomTR()
{
	...
	{
		ChangeTeam(client, 2);
		PrintToChatAll("{green}[DEATHRUN]{default} %t", "RandomTR");
	}
}
...
public Action Suicide(int client, const char[] command, int args)
{
	...
	PrintToChat(client, "{green}[DEATHRUN] {default}%t.", "KillPrevent");
	return Plugin_Handled;
}
